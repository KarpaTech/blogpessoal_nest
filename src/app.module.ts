...
@Module({
  imports: [
    TypeOrmModule.forRoot({
      ...
      entities: [Postagem, Tema], //A propriedade entities é configurada inicialmente com um array vazio. Após a criação das Classes Entidade, elas serão adicionadas a esse array para que o TypeORM possa gerar as tabelas correspondentes no banco de dados. | Dentro da configuração do banco de dados, no array entities, devemos importar todas as Classes Entidade do módulo. O TypeORM precisa dessas entidades para gerar a estrutura das tabelas no banco de dados. Aqui, estamos incluindo a Classe Postagem para que a tabela tb_postagens seja criada e manipulada corretamente
